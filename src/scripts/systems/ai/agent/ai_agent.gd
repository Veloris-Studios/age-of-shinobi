...
func _initialize_components() -> void:
	...
	# Check if this agent already has an AnimatedSprite2D child (built into the scene)
	var has_built_in_visuals = false
	for child in get_children():
		if child is AnimatedSprite2D:
			has_built_in_visuals = true
			break
	...
	# Get AnimatedSprite2D from scene (try multiple locations)
	if has_node("AnimatedSprite2D"):
		animated_sprite = $AnimatedSprite2D
	elif has_node("Visuals/AnimatedSprite2D"):
		animated_sprite = $Visuals/AnimatedSprite2D

	if not animated_sprite:
		push_warning("No AnimatedSprite2D found for %s" % species_id)
	else:
		# Ensure sprite is visible on spawn
...
func _apply_tier_config() -> void:
	...
	# Animation settings (AnimatedSprite2D doesn't have enabled property)
	if animated_sprite:
		animated_sprite.visible = tier_config.animation_enabled
...
func execute_action(intent: ActionIntent) -> void:
	...
	match intent.action:
		ActionIntent.ActionType.IDLE:
			if animated_sprite:
				animated_sprite.play("idle" + direction_suffix)
		...
		ActionIntent.ActionType.WANDER:
			...
			if animated_sprite:
				...
				animated_sprite.play("walk" + wander_suffix)
		...
		ActionIntent.ActionType.MOVE:
			...
			if animated_sprite:
				...
				animated_sprite.play("walk" + move_suffix)

		ActionIntent.ActionType.ATTACK:
			_perform_attack(intent.target)
			if animated_sprite:
				animated_sprite.play("attack" + direction_suffix)
			is_in_combat = true
		...
		ActionIntent.ActionType.FLEE:
			...
			if animated_sprite:
				...
				animated_sprite.play("run" + flee_suffix)
